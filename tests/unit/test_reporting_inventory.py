...
def test_the_document_figures_match_the_measurement(report):
    ...
    assert sum(m["lines"] for m in report["modules"]) == 9026
    assert counts["façade"] == 0
    assert counts["orchestration_i3f"] == 12 and lines["orchestration_i3f"] == 6207
    assert counts["lié_livrable_i3f"] == 9 and lines["lié_livrable_i3f"] == 2812
